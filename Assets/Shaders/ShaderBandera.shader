Shader "Custom/FlagLighting"
{
    Properties
    {
        _MainTex ("Bandera (Textura)", 2D) = "white" {}
        _Color ("Color Base", Color) = (1,1,1,1)
        _AmbientColor ("Luz Ambiental", Color) = (0.2,0.2,0.2,1)
        _LightColor ("Luz Direccional", Color) = (1,1,1,1)
        _LightDirection ("Dirección de Luz", Vector) = (0, -1, -1, 0)
        _SpecColor ("Color Especular", Color) = (1,1,1,1)
        _Shininess ("Brillo (Shininess)", Range(1,128)) = 32
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float3 L = normalize(-_LightDirection.xyz);   // dirección de la luz
                float3 V = normalize(_WorldSpaceCameraPos - i.worldPos); // dirección hacia la cámara
